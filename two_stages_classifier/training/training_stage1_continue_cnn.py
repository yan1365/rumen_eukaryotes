...
path = "/fs/ess/PAS0439/MING/cilates_fungi_classifier/outputs/GutEuk_classifier/stage1/cnn_v5_dropout0.5_wd0.001/"
...
model = utils.cnn_v5(BATCH)
model.to(device)
criterion  = nn.CrossEntropyLoss()
optimizer = torch.optim.AdamW(model.parameters(), weight_decay=0.001, lr=0.0001) 
...
for epoch in tqdm(range(num_epochs), desc = "Epochs finished"):
    ...
    start = time.time()
    train_batches = 0
    for index in tqdm(range(160), desc = "Batchs (training) finished", miniters=16): 
        train = utils.mydataset_m2("/fs/scratch/PAS0439/Ming/databases/gut_eukaryotes_classifier/transformed_dataset/train", str(index).zfill(3), BATCH)
    ...
    with torch.inference_mode():
        for index in tqdm(range(40), desc = "Batchs (validation) finished", miniters=4): 
            val = utils.mydataset_m2("/fs/scratch/PAS0439/Ming/databases/gut_eukaryotes_classifier/transformed_dataset/val", str(index).zfill(3), BATCH)   
